...
class WebnusTwitterFeed extends WP_Widget{
	...
	function __construct(){
		...
		$params = array(
		'description'=> __( "Twitter Feed", 'WEBNUS_TEXT_DOMAIN'),
		'name'=> __( "Webnus-Twitter Feed", 'WEBNUS_TEXT_DOMAIN')
		);
	...
	public function form($instance){
		...
		<p>
		<label for="<?php echo $this->get_field_id('username') ?>"><?php esc_html_e('Twitter User Name','WEBNUS_TEXT_DOMAIN'); ?>:</label>
		<input
		type="text"
		class="widefat"
		...
		<p>
		<label for="<?php echo $this->get_field_id('count') ?>"><?php esc_html_e('Feed Count','WEBNUS_TEXT_DOMAIN'); ?>:</label>
		<input
		type="text"
		class="widefat"
		...
		<p>
		<label for="<?php echo $this->get_field_id('access_token') ?>"><?php esc_html_e('Access Token','WEBNUS_TEXT_DOMAIN'); ?>:</label>
		<input
		type="text"
		class="widefat"
		...
		<p>
		<label for="<?php echo $this->get_field_id('access_token_secret') ?>"><?php esc_html_e('Access Token Secret','WEBNUS_TEXT_DOMAIN'); ?>:</label>
		<input
		type="text"
		class="widefat"
		...
		<p>
		<label for="<?php echo $this->get_field_id('consumer_key') ?>"><?php esc_html_e('Consumer Key','WEBNUS_TEXT_DOMAIN'); ?>:</label>
		<input
		type="text"
		class="widefat"
		...
		<p>
		<label for="<?php echo $this->get_field_id('consumer_secret') ?>"><?php esc_html_e('Consumer Secret','WEBNUS_TEXT_DOMAIN'); ?>:</label>
		<input
		type="text"
		class="widefat"
		...
		?>
			...
			<div class="lts-tweets">
				...
				if(isset($string["errors"][0]["message"])) {
					if($string["errors"][0]["message"] != "") {echo "<strong>".__( "Sorry, there was a problem.", 'WEBNUS_TEXT_DOMAIN')."</strong><p>".__( "Twitter returned the following error message:", 'WEBNUS_TEXT_DOMAIN')."</p><p><em>".$string["errors"][0]["message"]."</em></p>";}
				} else {
