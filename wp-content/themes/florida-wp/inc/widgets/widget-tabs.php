...
class WebnusWidgetTabs extends WP_Widget{
	...
	function __construct(){
		...
		$params = array(
		'description'=> __( "Webnus Widget Tabs", 'WEBNUS_TEXT_DOMAIN'),
		'name'=> __( "Webnus-Widget-Tabs", 'WEBNUS_TEXT_DOMAIN')
		);
	...
	public function form($instance){
		...
		<p>
			<label for="<?php echo $this->get_field_id('orderby'); ?>"><?php esc_html_e('Popular Posts Order By','WEBNUS_TEXT_DOMAIN'); ?>:</label>
			<select id="<?php echo $this->get_field_id('orderby'); ?>" name="<?php echo $this->get_field_name('orderby'); ?>" class="widefat" style="width:100%;">
				<option <?php if ('Comments' == $instance['orderby']) echo 'selected="selected"'; ?>><?php esc_html_e('Comments','WEBNUS_TEXT_DOMAIN'); ?></option>
				<option <?php if ('Views' == $instance['orderby']) echo 'selected="selected"'; ?>><?php esc_html_e('Views','WEBNUS_TEXT_DOMAIN'); ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('posts'); ?>"><?php esc_html_e('Number of popular posts','WEBNUS_TEXT_DOMAIN'); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('tags'); ?>"><?php esc_html_e('Number of recent posts','WEBNUS_TEXT_DOMAIN'); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('comments'); ?>"><?php esc_html_e('Number of comments','WEBNUS_TEXT_DOMAIN'); ?>:</label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('show_popular_posts'); ?>"><?php esc_html_e('Show popular posts','WEBNUS_TEXT_DOMAIN'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo $this->get_field_id('show_recent_posts'); ?>"><?php esc_html_e('Show recent posts','WEBNUS_TEXT_DOMAIN'); ?></label>
		</p>
		<p>
			<input class="checkbox" type="checkbox" <?php checked($instance['show_comments'], 'on'); ?> id="<?php echo $this->get_field_id('show_comments'); ?>" name="<?php echo $this->get_field_name('show_comments'); ?>" />
			<label for="<?php echo $this->get_field_id('show_comments'); ?>"><?php esc_html_e('Show comments','WEBNUS_TEXT_DOMAIN'); ?></label>
		</p>
		<?php 
	}
		...
		<div class="widget-tabs">
			<div class="tab-hold tabs-wrapper">
				<ul id="tabs" class="tabset tabs">
					...
					<li><a href="#tab-popular"><?php esc_html_e('Popular','WEBNUS_TEXT_DOMAIN'); ?></a></li>
					<?php endif; ?>
					<?php if($show_recent_posts == 'true'): ?>
					<li><a href="#tab-recent"><?php esc_html_e('Recent','WEBNUS_TEXT_DOMAIN'); ?></a></li>
					<?php endif; ?>
					<?php if($show_comments == 'true'): ?>
					<li><a href="#tab-comments"><?php esc_html_e('Comments','WEBNUS_TEXT_DOMAIN'); ?></a></li>
					<?php endif; ?>
				</ul>
					...
					<div id="tab-comments" class="tab tab_content" style="display: none;">
						<ul class="tab-list">
							...
							<li>
								...
								<div class="content">
									<p><?php echo strip_tags($comment->comment_author); ?> <?php esc_html_e('says','WEBNUS_TEXT_DOMAIN'); ?>:</p>
									<div>
										<a class="comment-text-side" href="<?php echo get_permalink($comment->ID); ?>#comment-<?php echo $comment->comment_ID; ?>" title="<?php echo strip_tags($comment->comment_author); ?> <?php esc_html_e('on','WEBNUS_TEXT_DOMAIN'); ?> <?php echo $comment->post_title; ?>"><?php echo strip_tags($comment->com_excerpt); ?>...</a>
									</div>
								</div>
							</li>
							<?php } ?>
