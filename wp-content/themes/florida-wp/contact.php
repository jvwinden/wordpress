...
if($show_titlebar && ( 'show' == $show_titlebar)): ?>
	<section id="headline" style="<?php if(!empty($titlebar_bg)) echo ' background-color:'.$titlebar_bg.';'; ?>">
	    <div class="container">
	      <h2 style="<?php if(!empty($titlebar_fg)) echo ' color:'.$titlebar_fg.';';  ?>"><?php the_title(); ?></h2>
	    </div>
	</section>
<?php endif; ?>
...
<section id="main-content" class="container">
	...
	<div class="col-md-5 contact-inf">
		<h4><?php echo __('Contact Information','WEBNUS_TEXT_DOMAIN') ?>:</h4><br>
		<p><strong><?php _e('Address','WEBNUS_TEXT_DOMAIN') ?>:</strong></p>
		<p><?php echo $webnus_options->webnus_contact_address(); ?></p><br>
		<p><strong><?php _e('Phone','WEBNUS_TEXT_DOMAIN') ?>:</strong></p>
		<p><?php echo $webnus_options->webnus_contact_phone(); ?><br></p><br>
		<p><strong><?php _e('Email', 'WEBNUS_TEXT_DOMAIN'); ?>:</strong></p>
		<p><?php echo $webnus_options->webnus_contact_email(); ?><br></p><br>
		<hr class="boldbx">
		<?php 
		...
		<div class="contact-form">
			...
			<form action="<?php the_permalink(); ?>" method="POST" id="contact-form" class="frmContact" role="form" novalidate>
				<div class="field-group">
					...
					<input type="text" name="cName" class="txbx" placeholder="<?php _e('Name','WEBNUS_TEXT_DOMAIN'); ?>" value="<?php if ( isset( $_POST['cName'] ) ) { echo $_POST['cName']; } ?>" /><br>
					<?php if ( isset( $errors['errorName'] ) ) : ?>
						<span class="bad-field"><?php echo $errors['errorName']; ?></span>
					<?php endif; ?>
				</div>

				<div class="field-group">
					<h5><?php _e('Whats your Email?','WEBNUS_TEXT_DOMAIN') ?></h5>
					<input  type="text" name="cEmail" class="txbx" placeholder="<?php _e('Email','WEBNUS_TEXT_DOMAIN'); ?>" value="<?php if ( isset( $_POST['cEmail'] ) ) { echo $_POST['cEmail']; } ?>" /><br>
					<?php if ( isset( $errors['errorEmail'] ) ) : ?>
						<span class="bad-field"><?php echo $errors['errorEmail']; ?></span>
					<?php endif; ?>
				</div>

				<div class="field-group">
					<h5><?php _e('Email Subject?','WEBNUS_TEXT_DOMAIN'); ?></h5>
					<input name="cSubject" type="text" class="txbx" placeholder="<?php _e('Subject','WEBNUS_TEXT_DOMAIN'); ?>" value="<?php if ( isset( $_POST['cSubject'] ) ) { echo $_POST['cSubject']; } ?>" /><br>
				</div>

				<div class="erabox">
					<div class="field-group">
						<h5><?php _e('Message to us','WEBNUS_TEXT_DOMAIN') ?></h5>
						<textarea name="cMessage" class="txbx era"><?php if ( isset( $_POST['cMessage'] ) ) { echo $_POST['cMessage']; } ?></textarea><br>
						<?php if ( isset( $errors['errorMessage'] ) ) : ?>
							<span class="bad-field"><?php echo $errors['errorMessage']; ?></span>
						<?php endif; ?>
					</div>
					
					<?php if ( $recaptcha_desire ) : ?>
						<?php if ( isset( $errors['errorreCaptcha'] ) ) : ?>
							<span class="bad-field captcha"><?php echo $errors['errorreCaptcha']; ?></span>
						<?php endif; ?>
					...
					<button type="submit" class="sendbtn btnSend" ><?php _e('Send Message','WEBNUS_TEXT_DOMAIN') ?></button>


					<?php if ( isset( $emailIsSent ) && $emailIsSent ) { ?>
						<div class="alert alert-success">
							<?php _e( 'Your message has been sucessfully sent, thank you!', 'WEBNUS_TEXT_DOMAIN' ); ?>
						</div> <!-- end alert -->
					<?php } elseif ( isset( $isError ) && $isError ) { ?>
						<div class="alert-alert-danger">
							<?php _e( 'Sorry, it seems there was an error.', 'WEBNUS_TEXT_DOMAIN' ); ?>
						</div> <!-- end alert -->
					<?php } ?>
